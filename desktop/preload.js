...
contextBridge.exposeInMainWorld("camargo", {
  ...
  update: {
    ...
    download: () => ipcRenderer.invoke("camargo:update-download"),
    install: () => ipcRenderer.invoke("camargo:update-install"),
    onStateChange: (callback) =>
      ipcRenderer.on("camargo:update-state", (_event, state) => callback(state)),
  },
});
