...
UGG_REGIONS = {
    ...
    "lan": "la1", "las": "la2", "na": "na1", "oce": "oc1", "ru": "ru",
    "tr": "tr1", "ph": "ph2", "sg": "sg2", "th": "th2", "tw": "tw2", "vn": "vn2",
}
...
def build_reveal_url(provider, region, summoner_names):
    ...
    if provider == "porofessor":
        ...
        return f"https://porofessor.gg/pregame/{region}/{players}/soloqueue/season"
    if provider == "opgg":
        players = quote(",".join(summoner_names), safe=",")
    ...
    for riot_id in summoner_names:
        ...
        ugg_names.append(f"{game_name}-{tag_line}" if separator else riot_id)
...
class Reveal(Feature):
    ...
    def __init__(self, lcu_client, config, on_event=None):
        ...
        self.provider = self.config["lobby_reveal"].get("provider", "porofessor")

    def get_status(self) -> dict:
        return {"key": self.key, "provider": self.provider}

    def set_provider(self, provider):
        if provider not in REVEAL_PROVIDERS:
            raise ValueError("Unsupported Lobby Reveal provider")
        self.provider = provider
        self.config["lobby_reveal"]["provider"] = provider
        save_config(self.config)
        return self.provider

    def build_url(self):
        champ_select = self.lcu.lcu_request("GET", "/lol-champ-select/v1/session")
        ...
        summoner_names = []
        hidden_names = any(
            player.get("nameVisibilityType") == "HIDDEN" for player in session.get("myTeam", [])
        )

        if hidden_names:
            participants = self.lcu.riot_request("GET", "/chat/v5/participants")
            for participant in participants.json().get("participants", []):
                if "champ-select" in participant.get("cid", ""):
                    summoner_names.append(f"{participant['game_name']}#{participant['game_tag']}")
        else:
            for player in session.get("myTeam", []):
                ...
                if response.status_code == 200:
                    ...
                    summoner_names.append(f"{summoner['gameName']}#{summoner['tagLine']}")

        region_response = self.lcu.lcu_request("GET", "/riotclient/region-locale")
        ...
        url = build_reveal_url(self.provider, region, summoner_names)
        self.on_event("success", f"Lobby reveal ready: {url}")
        return url
